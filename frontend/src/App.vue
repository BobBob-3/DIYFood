    ...
    <nav class="navbar-custom fixed-custom" style="background-color: #87a46a;">
      <div>
        <div v-if="cookie">
          ...
          <button class="btn btn-custom" style="margin-right: 70px;" type="button" onclick="location.href='/user/login'">LOG OUT</button>
          <button class="btn btn-custom" type="button" onclick="location.href='/user/mypage'">MY PAGE</button>
        </div>
        <div v-else>  <!--쿠키가 없다면 보여줄 목록-->
          <button class="btn btn-custom" style="margin-right: 70px;" type="button" onclick="location.href='/user/signup'">SIGN UP</button>
          <button class="btn btn-custom" type="button" onclick="location.href='/user/login'">LOG IN</button>
        </div>
      </div>
    </nav>
    <!-- Navigation Bar -->
